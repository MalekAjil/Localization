...
void Kalman::init(void)
{
	...
	P_[0][0] = 0.1; P_[0][1] = 0; P_[0][2] = 0;
	P_[1][0] = 0; P_[1][1] = 0.1; P_[1][2] = 0;
	P_[2][0] = 0; P_[2][1] = 0; P_[2][2] = 0.1;

	R[0][0] = rx; R[0][1] = 0; R[0][2] = 0;
	R[1][0] = 0; R[1][1] = ry; R[1][2] = 0;
	R[2][0] = 0; R[2][1] = 0; R[2][2] = rt;
...
P=(I-k*H)*p
*/
void Kalman::kalman(double w[2], double Z[3], double x_new[3])
{
	//(1-1) project the state ahead : Xhat(k)_=f(Xhat(k-1),u(k-1),0)
	//initialize Xhat
	Xhat_[0] += dt*w[0] * cos(Xhat_[2] * PI / 180);
	Xhat_[1] += dt*w[0] * sin(Xhat_[2] * PI / 180);
	Xhat_[2] += dt*w[1];

	//initialize F
	F[0][2] = -dt*w[0] * sin(Xhat_[2] * PI / 180);
	F[1][2] = dt*w[0] * cos(Xhat_[2] * PI / 180);

	//initialize Fu
	Fu[0][0] = dt*cos(Xhat_[2] * PI / 180);
	Fu[1][0] = dt*sin(Xhat_[2] * PI / 180);
	Fu[2][1] = dt;
	Fu[0][0] = 1;
	Fu[1][1] = 1;
	Fu[2][1] = 0;
	...
	mul_arr31(H, Xhat_, X1);
	sub_arr3(Z, X1, X2);
	mul_arr31(K, X2, X1);
	sum_arr3(Xhat_, X1, Xhat);
